...
export default function OrdersPage() {
  ...
  const getStatusInfo = (status: string) => {
    const map: Record<string, { label: string; color: string; icon: JSX.Element }> = {
      en_attente:     { label: 'En attente',     color: 'bg-yellow-100 text-yellow-700', icon: <Clock size={14} className="mr-1" /> },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/account" className="flex items-center gap-2 text-gray-600 hover:text-emerald-600 transition">
            <ArrowLeft size={20} /><span className="font-medium">Mon compte</span>
          </Link>
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4 mt-4">
            {orders.map((order) => {
              ...
              return (
                <div key={order.firestoreId} className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition border border-gray-100">
                  ...
                  <div className="border-t border-gray-100 pt-3 mt-3">
                    <div className="flex justify-between items-center mb-3">
                      <div>
                        ...
                        <p className="font-bold text-emerald-600 text-lg">{order.total?.toLocaleString()} FCFA</p>
                      </div>
                      {order.status === 'livree' && (
                      ...
                      <div className="mt-4">
                        <DeliveryTracker
                          steps={DELIVERY_STEPS.map((step, idx) => ({
                            ...step,
                            completed: order.deliverySteps[step.label.toLowerCase().replace(/ /g, '_').replace(/[^a-z_]/g, '')]?.completed || false,
                            timestamp: order.deliverySteps[step.label.toLowerCase().replace(/ /g, '_').replace(/[^a-z_]/g, '')]?.timestamp || null,
                          }))}
                      ...
                      {order.status === 'annulee' && (
                        <Link href="/main/products" className="flex-1">
                          <button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition">
                            <Package size={16} />Commander à nouveau
                          </button>
                        </Link>
                      )}
                    </div>
